...
-- Bug screenshots table
CREATE TABLE IF NOT EXISTS bug_screenshots (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    image TEXT NOT NULL,
    created TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    bug INTEGER NOT NULL,
    FOREIGN KEY (bug) REFERENCES bugs(id) ON DELETE CASCADE
);

-- Bug tags junction table (many-to-many)
...
-- Bug team members junction table (many-to-many)
-- Note: FK to user will be added in migration 0003
CREATE TABLE IF NOT EXISTS bug_team_members (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    bug_id INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_bugs_user ON bugs(user);
CREATE INDEX IF NOT EXISTS idx_bugs_hunt ON bugs(hunt);
CREATE INDEX IF NOT EXISTS idx_bugs_created ON bugs(created);
CREATE INDEX IF NOT EXISTS idx_bugs_status ON bugs(status);
CREATE INDEX IF NOT EXISTS idx_bugs_verified ON bugs(verified);
CREATE INDEX IF NOT EXISTS idx_bug_screenshots_bug ON bug_screenshots(bug);
CREATE INDEX IF NOT EXISTS idx_bug_tags_bug ON bug_tags(bug_id);
CREATE INDEX IF NOT EXISTS idx_bug_tags_tag ON bug_tags(tag_id);
CREATE INDEX IF NOT EXISTS idx_bug_team_members_bug ON bug_team_members(bug_id);
CREATE INDEX IF NOT EXISTS idx_bug_team_members_user ON bug_team_members(user_id);

-- Triggers to update modified timestamp automatically
CREATE TRIGGER IF NOT EXISTS update_bugs_modified 
AFTER UPDATE ON bugs
BEGIN
    UPDATE bugs SET modified = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;
